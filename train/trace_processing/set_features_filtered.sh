...
for file in $FILES; do
    ...
    # - eth.type (raw, supports IPv4/IPv6)
    ...
    # Support both IPv4 and IPv6 to preserve eth.type diversity.
    tshark -o tcp.analyze_sequence_numbers:TRUE \
      -r ${file} \
      -Y "(eth.type==0x0800 or eth.type==0x86dd) and (tcp or udp or icmp or icmpv6 or igmp)" \
      -Tfields -E occurrence=f -E separator=, \
      -e ip.len -e ipv6.plen -e eth.type -e ip.proto -e ipv6.nxt -e tcp.srcport -e tcp.dstport -e udp.srcport -e udp.dstport -e eth.src | \
      awk -F, 'BEGIN {OFS=","} {
      len=0;
      if ($1!="") len=$1;             # IPv4 total length
      else if ($2!="") len=$2+40;     # IPv6 payload + 40 bytes header
      else if ($3=="0x0806") len=28;  # ARP fallback (rare due to filter)

      # Map protocol to 5 buckets (ip.proto for IPv4, ipv6.nxt for IPv6)
      proto=$4; if (proto=="" && $5!="") proto=$5;
      pb=0;                 # OTHER default
      if (proto==6) pb=6;   # TCP
      ...
      sport=$6; dport=$7; usport=$8; udport=$9;
