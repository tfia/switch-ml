...
/* SwitchEgress.te_get_eg_port_info:
 *   key: eg_intr_md.egress_port
 *   data: port_ip, port_mac
 */
typedef struct te_get_eg_port_info_entry_s {
    bf_dev_port_t egress_port;   
    const char   *port_ip;       
    const char   *port_mac;      
    char*          action;    
} te_get_eg_port_info_entry_t;

/* SwitchEgress.te_set_dst_mac:
 *   key: eg_intr_md.egress_port + hdr.ipv4.dst_ip
 *   data: mac
 */
typedef struct te_set_dst_mac_entry_s {
    bf_dev_port_t egress_port;   
    const char   *dst_ip;        
    const char   *dst_mac;       
    char          action[16];    
} te_set_dst_mac_entry_t;
